...
export default function Index({ suppliers, filters }: PageProps) {
    ...
    const toggleSort = (column: keyof Supplier) => {
    ...
    const handleEdit = (supplier: Supplier) => setEditingSupplier(supplier);
    ...
    return (
        <AppLayout breadcrumbs={isMobile ? [] : breadcrumbs}>
            ...
            <div className="space-y-6 p-4 sm:p-6 lg:p-8">
                ...
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Gérez les fournisseurs.</p>
                    </div>
                    <div className="flex gap-2 w-full md:w-auto">
                        <div className='relative'>
                            <Search className="absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                            <Input
                                placeholder={isMobile ? "Rechercher..." : "Rechercher un fournisseur..."}
                                value={search}
            ...
            <SupplierForm
                ...
                submitUrl={
                    editingSupplier
                        ? route('suppliers.update', editingSupplier.id)
            ...
            <ConfirmDeleteDialog
                ...
                message={deleteSupplier
                    ? `Voulez-vous vraiment supprimer ${deleteSupplier.name} ? Cette action est irréversible.`
                    : `Voulez-vous vraiment supprimer ${Object.keys(rowSelection).length} fournisseur(s) ? Cette action est irréversible.`}
                onConfirm={() => {
                    const ids = deleteSupplier ? [deleteSupplier.id]
                                ...
                                toast.success(
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-sm text-muted-foreground">
                                            Les fournisseurs sélectionnés ont été supprimés.
                                        </span>
                                    </div>
                                );
                            },
